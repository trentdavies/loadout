...
test_skill_show() {
  ...
  assert_exit_code 0 "$LOADOUT" list src:test-plugin/explore
  assert_stdout_contains "explore" "$LOADOUT" list src:test-plugin/explore
  assert_stdout_contains "Description" "$LOADOUT" list src:test-plugin/explore
}
...
test_skill_show_displays_source_info() {
  ...
  "$LOADOUT" add "$FIXTURES_DIR/plugin-source" --source src >/dev/null 2>&1
  local output
  output=$("$LOADOUT" list src:test-plugin/explore 2>/dev/null)
...
test_skill_show_nonexistent() {
  ...
  local output
  output=$("$LOADOUT" list src:test-plugin/nonexistent 2>&1)
