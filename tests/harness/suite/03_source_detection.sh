...
test_detect_single_file() {
  ...
  assert_exit_code 0 "$SKITTLE" source add "$FIXTURES_DIR/single-skill/SKILL.md" --name single
  # Should result in one skill
  local output
  output=$("$SKITTLE" skill list 2>/dev/null)
  if echo "$output" | grep -qF "single-skill"; then
    _pass "single file detected as skill"
  else
    _fail "single file not detected" "single-skill in skill list" "$output"
  fi
}

test_detect_flat_directory() {
  "$SKITTLE" init >/dev/null 2>&1
  assert_exit_code 0 "$SKITTLE" source add "$FIXTURES_DIR/flat-skills" --name flat
...
test_detect_plugin_directory() {
  "$SKITTLE" init >/dev/null 2>&1
  assert_exit_code 0 "$SKITTLE" source add "$FIXTURES_DIR/plugin-source" --name plugged
...
test_detect_full_source() {
  "$SKITTLE" init >/dev/null 2>&1
  assert_exit_code 0 "$SKITTLE" source add "$FIXTURES_DIR/full-source" --name full
  # Should detect 2 plugins
...
test_detect_unrecognizable_directory() {
  ...
  assert_exit_code 1 "$SKITTLE" source add "$FIXTURES_DIR/invalid/empty-dir" --name bad
  assert_stderr_contains "error" "$SKITTLE" source add "$FIXTURES_DIR/invalid/empty-dir" --name bad
}
...
test_detect_invalid_no_frontmatter_warns() {
  ...
  local output
  output=$("$SKITTLE" source add "$FIXTURES_DIR/invalid/no-frontmatter/SKILL.md" --name nofm 2>&1)
  local exit_code=$?
...
test_detect_invalid_bad_name_warns() {
  ...
  local output
  output=$("$SKITTLE" source add "$FIXTURES_DIR/invalid/bad-name/SKILL.md" --name badname 2>&1)
  local exit_code=$?
...
test_detect_name_derived_from_directory() {
  ...
  assert_exit_code 0 "$SKITTLE" source add "$FIXTURES_DIR/flat-skills"
