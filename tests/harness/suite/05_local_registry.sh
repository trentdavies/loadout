...
test_skill_identity_full_form() {
  ...
  assert_exit_code 0 "$SKITTLE" skill show tp:test-plugin/explore
}
...
test_skill_identity_ambiguous_error() {
  ...
  "$SKITTLE" source add "$FIXTURES_DIR/plugin-source" --name src-one >/dev/null 2>&1
  "$SKITTLE" source add "$FIXTURES_DIR/plugin-source" --name src-two >/dev/null 2>&1
  # Short form should be ambiguous now
  local output
  output=$("$SKITTLE" skill show test-plugin/explore 2>&1)
  local exit_code=$?
...
test_xdg_override_respected() {
  ...
  XDG_DATA_HOME="$custom_data" "$SKITTLE" source add "$FIXTURES_DIR/plugin-source" --name tp >/dev/null 2>&1
