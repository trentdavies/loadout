...
The adapter SHALL compare a source skill directory against its installed counterpart at the agent and return one of three statuses: NEW (not present at agent), UNCHANGED (all files match byte-for-byte), or CHANGED (at least one file differs in content, is missing at agent, or exists only at source).

#### Scenario: Skill not present at agent
- **WHEN** the source skill directory has no corresponding directory at the agent
- **THEN** the adapter SHALL return status NEW

#### Scenario: Skill identical at agent
- **WHEN** every file in the source skill directory has a byte-identical counterpart at the agent, and no extra files exist at either location
- **THEN** the adapter SHALL return status UNCHANGED

#### Scenario: Skill file content differs
- **WHEN** at least one file in the source skill directory has different content than its counterpart at the agent
- **THEN** the adapter SHALL return status CHANGED

#### Scenario: Source has file not present at agent
- **WHEN** the source skill directory contains a file that does not exist at the agent
- **THEN** the adapter SHALL return status CHANGED

#### Scenario: Agent has extra file not in source
- **WHEN** the agent skill directory contains a file that does not exist in the source
- **THEN** the adapter SHALL return status CHANGED
...
- **WHEN** SKILL.md and scripts/run.sh both differ between source and agent
...
- **THEN** the skill SHALL be overwritten at the agent without prompting
