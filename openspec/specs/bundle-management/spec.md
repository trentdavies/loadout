...
The CLI SHALL support `skittle bundle list` to display all bundles with their skill count and which targets they're active on.
...
The CLI SHALL support `skittle bundle add <bundle> <skill...>` to add one or more skills to a bundle. Skills are specified as `plugin/skill` identifiers. Glob patterns (e.g., `openspec/*`) SHALL be supported to add all skills from a plugin.
...
- **THEN** all skills from the "openspec" plugin SHALL be added to the bundle
