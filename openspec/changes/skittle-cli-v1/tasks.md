...
- [x] 2.4 Implement `skittle config show` (text + --json) and `skittle config edit` ($EDITOR/$VISUAL/vi fallback)
...
- [x] 4.9 Implement `skittle source update [name]` — re-fetch and update cache
...
- [x] 6.4 Implement `skittle skill list [--source] [--plugin]` and `skittle skill show <plugin/skill>` reading SKILL.md frontmatter for display
...
- [x] 12.2 Add integration tests: install/uninstall (--all, --skill, --plugin, --bundle), dry run, idempotency
